...
export default function ReviewStep({
  ...
  return (
    <div className='space-y-6'>
      <div>
        <Typography as='h2' weight='bold' className='mb-2 text-3xl text-white'>
          Tinjauan Akhir — Apakah Semuanya Sempurna?
        </Typography>
        <Typography as='p' className='text-white/80'>
      ...
      <Box delay={0.1}>
        ...
        <div className='space-y-3 text-sm'>
          <div className='flex items-center justify-between rounded px-2 py-2 transition-colors hover:bg-[#0040FF]/5'>
            <span className='font-medium text-white/70'>Nama:</span>
            <span className='font-semibold text-white'>
              {formData.nama || '-'}
            </span>
          </div>
          {errors?.nama && (
            <p className='px-2 text-xs text-red-400'>{errors.nama}</p>
          )}
          <div className='flex items-center justify-between rounded px-2 py-2 transition-colors hover:bg-[#0040FF]/5'>
            <span className='font-medium text-white/70'>NRP:</span>
            <span className='font-semibold text-white'>
              {formData.nrp || '-'}
            </span>
          </div>
          {errors?.nrp && (
            <p className='px-2 text-xs text-red-400'>{errors.nrp}</p>
          )}
          <div className='flex items-center justify-between rounded px-2 py-2 transition-colors hover:bg-[#0040FF]/5'>
            <span className='font-medium text-white/70'>Kelompok KP:</span>
            <span className='font-semibold text-white'>
              {formData.kelompokKP || '-'}
            </span>
          </div>
          {errors?.kelompokKP && (
            <p className='px-2 text-xs text-red-400'>{errors.kelompokKP}</p>
          )}
        </div>
      </Box>
      ...
      <Box delay={0.2}>
        <div className='mb-4 flex items-center gap-2'>
          ...
          <Typography as='h3' weight='semibold' className='text-white'>
            Bundle Dokumen (ZIP)
          </Typography>
        </div>
        <div className='space-y-3 text-sm'>
          <div className='flex items-center justify-between rounded px-2 py-2 transition-colors hover:bg-[#0040FF]/5'>
            <span className='font-medium text-white/70'>File ZIP:</span>
            <span className='font-semibold text-white'>
              {formData.file_zip
          ...
          {errors?.zipFile && (
            <p className='px-2 text-xs text-red-400'>{errors.zipFile}</p>
          )}
          <div className='rounded px-2 py-2 text-white/80'>
            <p className='text-xs'>Isi ZIP:</p>
      ...
      <Box delay={0.3}>
        ...
        <div className='space-y-4'>
          {[1, 2, 3].map((num) => (
            ...
            >
              <p className='mb-2 text-xs font-semibold text-[#66a3ff]'>
                Pertanyaan {num}
              </p>
      ...
      <Box delay={0.4}>
        ...
        <div className='flex flex-wrap gap-2'>
          {formData.selectedDivisions.map((divId, idx) => (
            <motion.span
              key={divId}
              className='rounded-full border border-[#0040FF]/40 bg-gradient-to-r from-[#0040FF]/20 to-blue-300/20 px-4 py-2 text-sm font-semibold text-[#66a3ff]'
                ...
                <div className='space-y-2'>
                  {[1, 2, 3, 4, 5].map((num) => (
                    <div key={num} className='text-sm'>
                      <p className='mb-1 text-xs font-semibold text-[#66a3ff]'>
                        Pertanyaan {num}
                      </p>
