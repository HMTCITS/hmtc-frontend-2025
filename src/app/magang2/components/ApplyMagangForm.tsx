...
export default function ApplyMagangForm({
  ...
  // Form refs for animation
  const formRef = React.useRef<HTMLFormElement>(null);
  const gridFieldsRef = React.useRef<HTMLDivElement>(null);
  const kelompokKPRef = React.useRef<HTMLDivElement>(null);
  const mindmapSectionRef = React.useRef<HTMLDivElement>(null);
  ...
  React.useEffect(() => {
    if (
      !gridFieldsRef.current ||
      !kelompokKPRef.current ||
      !mindmapSectionRef.current ||
      !buttonSectionRef.current
    )
      return;

    // Set initial states - all elements start invisible and offset
    gsap.set(
      [
        gridFieldsRef.current,
        kelompokKPRef.current,
    ...
    // Create timeline for sequential slide-in animations
    const tl = gsap.timeline();

    // Grid fields (nama & nrp)
    tl.to(gridFieldsRef.current, {
      opacity: 1,
      y: 0,
      duration: 0.8,
      ease: 'power2.out',
      ...
      >
        ...
        >
          <Input
            ...
            disabled={isBusy}
            autoComplete='name'
            labelClassName='text-white font-satoshi'
        ...
        <div ref={mindmapSectionRef} className='space-y-3'>
          <Typography
            as='label'
            htmlFor='mindmap'
            variant='s3'
        ...
        <div
          ...
          className='flex flex-col-reverse items-stretch justify-end gap-3 border-t border-white/10 pt-6 md:flex-row md:items-center'
        >
          <Button
            type='reset'
            variant='outline'
            ...
            className='w-full border-white/40 font-satoshi text-white hover:bg-white md:w-auto'
          >
            Reset
          </Button>
          <Button
