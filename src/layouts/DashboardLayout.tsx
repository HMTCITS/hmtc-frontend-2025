...
export default function DashboardLayout({
  ...
  useEffect(() => {
    // Render interactive children only after client mount to avoid
    // hydration mismatches caused by browser extensions mutating DOM
    // before React hydrates (e.g. extension-injected attributes).
    setMounted(true);
  }, []);
  ...
  return (
    <Providers>
      <div className='min-h-screen bg-[#EDFAFF]'>
        {/* Render header/sidebar/main only after client mount to avoid
            hydration mismatches from extension-injected attributes */}
        {mounted ? (
          <>
            {/* Header */}
            <HeaderBar
